...
export default function Accessories() {
  ...
  let allAccessoriesProducts =[];
  let subCategories;
  if(data && data.status){
    for(let accessories of data.body){
      if(accessories.productCategory.toLowerCase() === 'accessories'){
        allAccessoriesProducts.push(accessories);
      }
  ...
  if(categoryQuery.data && categoryQuery.data.status){
    for(let category of categoryQuery.data.body){
      if(category.category.categoryName.toLowerCase() === 'accessories'){
    ...
    <Layout>
     <CategoryNav styles="hidden md:flex border-b-[0.15rem] border-[#5E6368]">
      ...
      <div className="p-[3rem] sm:p-[6rem]">
        <ShopHero>
          <ShopHero.Name>Accessories</ShopHero.Name>
        ...
        <div className="flex flex-wrap mx-[-4%] sm:mx-[-3%] md:mx-[-1.4%] mb-[10rem] ">
          {allAccessoriesProducts.map((product) => (
              <ProductCard
